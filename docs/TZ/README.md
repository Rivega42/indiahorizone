...
Все задачи в трекере живут в **трёхуровневой иерархии**: эпик → issue → sub-issue. Подробно — [`CLAUDE.md` § 8](../../CLAUDE.md). Кратко: каждый issue имеет родителя-эпик; идея масштаба эпика → новый эпик; «голых» issues без родителя не бывает.

## Roadmap

Полный план исполнения backlog по неделям, MVP-точкам и production-готовности — [`docs/ROADMAP.md`](../ROADMAP.md).
...
| Вика | Devops AI, выполняет действия на серверах / в инфраструктуре по issues с тегом `[devops:vika]`. См. [`CLAUDE.md` § 7](../../CLAUDE.md). |
...
| Devops AI | Вика | Серверы, инфра, секреты, deploy, верификации (issues `[devops:vika]`) |
| Dev AI | ассистент Claude / dev-команда | Код, документация, архитектура, тесты |
