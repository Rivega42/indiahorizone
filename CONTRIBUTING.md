...
4. Нет подходящего issue — создайте новый **под существующим эпиком** (см. §6 ниже)

#### Иерархия issues — обязательная

Все задачи живут в иерархии **epic → issue → sub-issue**. Принцип закреплён в [`CLAUDE.md` § 8](CLAUDE.md). Кратко:

- **Эпик** (`[EPIC N] ...`) = стратегический блок продукта.
- **Issue** = конкретная фича/задача в рамках эпика. Любой новый issue должен иметь родителя-эпик. Привязка — через GitHub native sub-issue UI или через чек-лист в body эпика.
- **Sub-issue** = атомарный шаг внутри сложной issue (3+ PR'ов).

Перед `Create issue`:
1. Сверьтесь со списком эпиков в [`docs/TZ/README.md`](docs/TZ/README.md).
2. Если задача масштаба нового эпика — обсудите с founders, затем создайте эпик и issues под ним.
3. Если идея — issue под существующим эпиком, выберите primary parent.

«Голый» issue без родителя считается ошибкой и подлежит реорганизации.
